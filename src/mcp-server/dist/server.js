...
import { validateToken, updateResumeInDB, getResumeFromDB, analyzeResumeFit, sendTelegramNotification, logMcpCall } from './lib/auth';
import { RESUME_SCHEMA } from './types';
const server = new Server({ name: 'cv-sarkhan-dev-mcp', version: '1.0.0' }, { capabilities: { tools: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        return {
            isError: true,
            content: [{ type: 'text', text: JSON.stringify({ error: 'internal_error', message: err.message, duration: duration }) }],
        };
    }
});
async function main() {
